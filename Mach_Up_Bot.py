...
def get_story():
    ...
    if number == 2:
        with open ("Blackbird_Flyby.txt","r") as f:
            story = f.read()
    if number == 3:
        with open ("Blackbird_Crash.txt","r") as f:
            story = f.read()
    if number == 4:
...
def run_bot(r, comments_replied_to, submissions_replied_to):
    titlenumber = 25
    commentnumber = 100
    sublist = ['airforce', 'aviation', 'militaryporn', 'military', 'todayilearned', 'history', 'space', 'technology', 'pics', 'science']
    #sublist = ['test']
    for sub in sublist:
...
def main():
    ...
    run_bot(r, comments_replied_to, submissions_replied_to)
    logger("End")
